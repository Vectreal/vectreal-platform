...
`sharp` (`^0.34`) is **optional**, not required. Install it only to enable native server-side texture compression; without it, `compressTextures` falls back to basic optimization (see the note at the top of this page). Browser and edge callers inject their own encoder and need no native build.
