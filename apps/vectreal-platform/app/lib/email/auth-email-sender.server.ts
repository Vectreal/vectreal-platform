...
import type { AuthHookPayload, RawEmailActionType } from './auth-hook-verifier.server'

// ---------------------------------------------------------------------------
// Action alias normalization
// ---------------------------------------------------------------------------

const ACTION_ALIAS_TO_CANONICAL: Record<RawEmailActionType, CanonicalEmailAction> = {
  signup: 'signup',
  recovery: 'recovery',
  email: 'magic_link',
...
function canonicalizeAction(raw: RawEmailActionType): CanonicalEmailAction {
  return ACTION_ALIAS_TO_CANONICAL[raw]
}

function needsConfirmLink(
  action: CanonicalEmailAction,
): action is Exclude<CanonicalEmailAction, 'reauthentication' | 'password_changed_notification'> {
  return action !== 'reauthentication' && action !== 'password_changed_notification'
}
...
export async function sendAuthEmail(payload: AuthHookPayload): Promise<void> {
  ...
  const code = action === 'reauthentication' ? payload.email_data.token : undefined
