...
`?camera`, `?autoRotate` and `?transition` apply once, on `viewer_ready`. `?theme` and the three hotspot parameters are not applied at a moment at all - they describe how the viewer renders for as long as it is loaded, which is why suppressing the markers does not make them flash on screen first.
