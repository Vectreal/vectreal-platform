...
|---|---|---|
| `file` | `ModelFile \| null` | Loaded file metadata + Three.js model |
| `isFileLoading` | `boolean` | True while loading/parsing |
...
| `on` / `off` | Event helpers | Subscribe/unsubscribe to loader lifecycle events |
...
| `serverOptions` | `ServerOptions` | Endpoint/auth/header configuration |
...
| `sceneData` | `ServerSceneData` | Already-resolved payload containing glTF/settings/assets |
| `applySettings` | `boolean` | Whether scene settings are applied after load |
