...
export async function sendInternalContactNotification(args: {
	...
	if (!isResendConfigured()) {
		if (process.env.NODE_ENV !== 'production') {
			console.warn(
				'[contact] RESEND_API_KEY not set - skipping internal notification in dev'
			)
			return { ok: true }
		}
	}
	...
	try {
		...
		return {
			ok: false,
			error: `Failed to send internal notification: ${message}`
		}
	}
}
...
export async function sendSubmitterConfirmation(args: {
	...
	if (!isResendConfigured()) {
		if (process.env.NODE_ENV !== 'production') {
			console.warn(
				'[contact] RESEND_API_KEY not set - skipping confirmation email in dev'
			)
			return { ok: true }
		}
	}
