...
export const ToolSidebar = ({ user, isMobile = false }: ToolSidebarProps) => {
	...
	const handleTabChange = useCallback(
		(value: string) => {
			setProcessState((prev) => ({ ...prev, mode: value as SidebarMode }))
		},
		[setProcessState]
	)

	const toggleSidebar = useCallback(() => {
		setProcessState((prev) => ({ ...prev, showSidebar: !prev.showSidebar }))
	}, [setProcessState])

	const handleOpenChange = useCallback(
		(open: boolean) => {
			setProcessState((prev) => ({ ...prev, showSidebar: open }))
		},
		[setProcessState]
	)
