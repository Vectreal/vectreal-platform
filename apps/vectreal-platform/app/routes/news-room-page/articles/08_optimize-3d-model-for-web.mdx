...
coverImage: "/media/images/newsroom/og-optimize-3d-model-for-web.webp"
draft: false
sceneImage: "/media/images/newsroom/scene-optimize-3d-model-for-web.webp"
---
...
{/*
COVER IMAGE BRIEF - /media/images/newsroom/08/og-optimize-3d-model-for-web.webp
...
THUMBNAIL BRIEF - /media/images/newsroom/08/thumbnail-optimize-3d-model-for-web.webp
Aspect ratio: 4:3.
...
Open the [Vectreal Publisher](https://vectreal.com/publisher). Upload the GLB or glTF. The platform runs the inspect pass automatically and displays the before stats: file size, triangle count, texture sizes. Switch between the three optimization presets - Maximum quality, Balanced, Smallest - and the viewport re-renders from the original uploaded file each time. The stats panel updates with the after numbers. When the right tradeoff is found, publish.

The presets differ only in how hard they push textures: Maximum quality caps them at 2048 px and quality 90, Balanced at 1024/80, Smallest at 512/70. Geometry handling is identical in all three - duplicate removal plus Draco at publish - and mesh simplification stays off unless you enable it yourself in the advanced panel. The specific tradeoffs behind each preset, including when to override the defaults for specific material types, are covered in [Optimization Presets Demystified](https://vectreal.com/news-room/optimization-presets-and-texture-compression).
...
| Publisher Balanced preset, plus simplification enabled manually | 3.1 MB | 309,000 | Preset texture pass on top of the simplification above |
...
GLB is the right starting point for any web optimization pass. It is binary-packed and self-contained, with all external texture and buffer references resolved into a single file. Multi-file glTF (JSON + separate texture files) should be packed to GLB before optimizing. OBJ, FBX and STL need no conversion step of their own, because the loader converts them to glTF on the way in. USDZ is the exception in the other direction: it is something the Publisher writes, not something it reads.

The distinction between glTF and GLB, what each actually contains, and why it matters for the optimization step is covered in [glTF vs GLB: what actually matters when you ship to the web](https://vectreal.com/news-room/gltf-vs-glb-what-actually-matters).

---

The numbers are only convincing once you watch them drop on your own file. Upload one and read the stats panel:

Try it, no account needed: [vectreal.com/publisher](https://vectreal.com/publisher?utm_source=newsroom&utm_medium=article&utm_campaign=optimize-3d-model-for-web)
