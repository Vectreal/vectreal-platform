...
Those four are the whole list. OBJ, FBX and STL do not load, so convert to glTF or GLB first.
...
The viewer uses Drei's `Bounds` component to automatically fit the camera to the loaded model. The **Bounds margin** setting controls how tightly the camera frames the model - a smaller margin crops tighter, a larger value leaves more space around it.
...
The full walkthrough, including how to aim the light in the scene and why none of this costs your published embed anything, is in [adding shadows to a 3D model on your website](https://vectreal.com/news-room/adding-shadows-to-a-3d-model-on-your-website).
