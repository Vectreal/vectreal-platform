...
Browser-side React hooks for loading, optimizing, and exporting 3D models. The runtime counterpart to [`@vctrl/core`](/docs/packages/core) - built for React apps that need to handle 3D files directly in the browser.
...
|---|---|---|
| `useLoadModel` | `@vctrl/hooks/use-load-model` | Load and parse GLTF/GLB/USDZ files from file lists or dropped directories |
...
It returns the terminal state as well, for the callers that need to branch right
...
| `server` | `sceneId: string`, `serverOptions?`, `parseMode?` | Fetch a scene from the API by id |
...
| `file` | `ModelFile \| null` | Loaded file metadata + Three.js model. Non-null exactly when `status` is `'ready'` |
...
| `optimizer` | `OptimizerIntegrationReturn<true> \| null` | Populated when hook is called with `useOptimizeModel()` |

### Error codes

`error.code` is one of `unsupported_format`, `multiple_models`,
`binary_load_failed`, `gltf_load_failed`, `missing_assets`, `server_load_failed`,
`not_found`, `quota_exceeded`, `unknown`. Optimizer ingest is deliberately not
among them: the model is already on screen by then, so a failure there costs the
optimize step and is reported on `optimizer.error`.

---

## `useOptimizeModel`

Runs mesh simplification, deduplication, quantization, normals optimization, and texture compression using [glTF-Transform](https://gltf-transform.dev). Everything runs on the calling thread: the hook spawns no Web Worker. If you need the work off the main thread, run the hook inside a worker you own, as the Vectreal Platform does.
...
> `applyOptimization(fn, opts?)` is **not** a direct method on `useOptimizeModel`. It is exposed on the `optimizer` object returned by `useLoadModel(optimizer)` when you pass an optimizer instance in; it runs an optimization step and syncs the result back into loader state, as shown in the example above.
...
`texturesOptimization` takes `TextureCompressOptions` from `@vctrl/core/model-optimizer`. There is no server round-trip and no `serverOptions`: the hook injects a browser `OffscreenCanvas` encoder for you and compression happens entirely in the page.
...
> `three` is a **peer dependency** of `@vctrl/hooks`, and of `@vctrl/viewer`, so you install it yourself. **Declare it explicitly** in your own project's `package.json` to guarantee a single shared instance: Three.js uses global singletons internally and duplicate instances produce subtle rendering bugs.
>
> ```bash
> npm install three
> # or
...
> ```
>
> Let your package manager resolve the version against the declared peer range rather than pinning one here.
...
- [Uploading Models](/docs/guides/upload) - file ingestion and bundle handling
- [Optimizing & Configuring](/docs/guides/optimize) - optimization presets and behavior
- [@vctrl/viewer](/docs/packages/viewer) - rendering integration target
