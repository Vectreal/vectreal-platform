...
coverImage: "/media/images/newsroom/og-how-to-add-3d-model-to-website.webp"
draft: false
sceneImage: "/media/images/newsroom/scene-how-to-add-3d-model-to-website.webp"
---
...
{/*
COVER IMAGE BRIEF - /media/images/newsroom/07/og-how-to-add-3d-model-to-website.webp
...
THUMBNAIL BRIEF - /media/images/newsroom/07/thumbnail-how-to-add-3d-model-to-website.webp
Aspect ratio: 4:3.
...
Open the [Vectreal Publisher](https://vectreal.com/publisher). Drag in a GLB, glTF, STL, FBX or OBJ file. The platform optimizes it automatically - WebP texture compression, duplicate removal, and Draco geometry compression at publish time - all in the same pipeline used by production deployments. Configure the camera angle, environment lighting, and background. Set which domains are allowed to display the embed. Publish.

What comes back is a stable scene URL and an iframe snippet:

```html
<iframe
  src="https://vectreal.com/embed/[project-id]/[scene-id]?token=[preview-api-key]"
  width="100%"
  height="500"
  frameborder="0"
...
export function ProductViewer({ model }: { model: Object3D }) {
  return (
    <VectrealViewer
      model={model}
      cameraOptions={{
        cameras: [
          { cameraId: 'default', name: 'Default', initial: true, position: [0, 1, 4] }
        ]
      }}
...
Whichever path fits your team, the Publisher is the fastest way to evaluate before you commit a line of code:

Try it, no account needed: [vectreal.com/publisher](https://vectreal.com/publisher?utm_source=newsroom&utm_medium=article&utm_campaign=how-to-add-3d-model-to-website)
