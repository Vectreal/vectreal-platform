 ...
 *
 * Texture compression is NOT handled here — it remains a server-side
 * concern routed through the Supabase Edge Function.
 *
 * Usage:
...
export type { WorkerOptimizationOptions, WorkerInputMessage, WorkerOutputMessage }
...
self.onmessage = async (event: MessageEvent<WorkerInputMessage>) => {
  ...
  optimizer.onProgress(({ operation, progress }) => {
    post({ type: 'progress', step: operation, progress })
  })
  ...
  try {
    ...
    post({ type: 'done', buffer: result.buffer as ArrayBuffer }, [result.buffer as ArrayBuffer])
  } catch (err) {
    post({
      type: 'error',
