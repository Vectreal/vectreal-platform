...
> `three` (`^0.177.0`) is a **direct dependency** of `@vctrl/hooks` and is bundled automatically. You do not need to declare it separately when using hooks alone, though `@vctrl/viewer` requires it as a peer dependency.
