...
| `optimizer` | `OptimizerIntegrationReturn \| null` | Populated when hook is called with `useOptimizeModel()` |
...
> `three` (`^0.177.0`) is a **peer dependency** of `@vctrl/hooks`, so you must install it in your own project. `@vctrl/viewer` also declares `three` as a peer dependency. **Declare `three` explicitly** in your own project's `package.json` to guarantee a single shared instance - Three.js uses global singletons internally and having duplicate instances produces subtle rendering bugs:
>
> ```bash
> npm install three@^0.177.0
> # or
> pnpm add three@^0.177.0
> ```
