...
resource "google_iam_workload_identity_pool" "github_actions" {
  ...
  description               = "Identity pool for keyless GitHub Actions OIDC authentication"
}
...
# Allow GitHub Actions (any ref in this repo) to impersonate the staging deployer SA
resource "google_service_account_iam_member" "staging_deployer_wif" {
  count              = var.enable_workload_identity ? 1 : 0
  service_account_id = google_service_account.staging_deployer.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.github_actions[0].name}/attribute.repository/${var.github_org}/${var.github_repo}"
}

# Allow GitHub Actions (any ref in this repo) to impersonate the production deployer SA
resource "google_service_account_iam_member" "prod_deployer_wif" {
  count              = var.enable_workload_identity ? 1 : 0
  service_account_id = google_service_account.prod_deployer.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.github_actions[0].name}/attribute.repository/${var.github_org}/${var.github_repo}"
