...
> **Declare `three` explicitly** in your own `package.json`. Three.js uses global
> singletons internally, so two copies produce rendering bugs that are hard to
> trace back to their cause.

It is a peer dependency of both `@vctrl/hooks` and `@vctrl/viewer`, so you
install it yourself:
...
Let your package manager resolve the version against the declared peer range
