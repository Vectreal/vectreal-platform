...
// ---------------------------------------------------------------------------
// Object override types
// ---------------------------------------------------------------------------

/** Per-material color and PBR overrides. */
export interface MaterialOverride {
	/** Material name as it appears in the GLTF asset. */
	materialName: string
...
/** Per-material texture slot overrides. */
export interface TextureOverride {
	materialName: string
	/**
	 * GLTF texture slot to override, e.g. 'baseColorTexture' | 'metallicRoughnessTexture'.
	 */
	slot: string
	/** Asset ID from the platform asset library or a fully qualified URL. */
	assetIdOrUrl: string
}

/**
 * Per-object transform and material/texture overrides keyed by stable node name.
 * Applied on top of the base GLTF without mutating the source asset.
 */
export interface ObjectOverride {
	/** Stable glTF node name. */
	nodeId: string
	name?: string
	/** Disabled overrides are persisted but not applied at runtime. */
	enabled: boolean
	transform?: {
...
// ---------------------------------------------------------------------------
// Placeable reference types
// ---------------------------------------------------------------------------

/**
 * An instance of a built-in placeable object inserted into the scene.
 * catalogId references an entry in the static placeables catalog.
 */
export interface PlaceableRef {
	id: string
	/** Identifier from the static built-in catalog, e.g. 'ground-plane' | 'waterdrop-backdrop'. */
	catalogId: string
	name?: string
	transform?: {
		position?: [number, number, number]
...
export interface SceneSettings {
	...
	/** Per-object node overrides (transform + material + texture). Stored as JSON in scene_settings. */
	objectOverrides?: ObjectOverride[]
	/** Built-in placeable object instances inserted into the scene. */
	placeables?: PlaceableRef[]
}
